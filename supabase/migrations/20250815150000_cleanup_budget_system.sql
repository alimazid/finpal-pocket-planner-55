-- Clean up budget system by removing legacy columns and starting fresh

-- Delete all existing budgets and budget categories to start clean
...
-- Update the spent calculation trigger to work without legacy period columns
CREATE OR REPLACE FUNCTION public.update_budget_spent_v3()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
CREATE TRIGGER update_budget_spent_trigger
    ...
    FOR EACH ROW
    EXECUTE FUNCTION public.update_budget_spent_v3();